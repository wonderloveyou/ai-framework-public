...
| Aspect | Value |
|--------|-------|
| **Input** | PR link, branch name, or `mode:*` flags; defaults to current branch diff |
| **Output** | Structured review report with severity-labelled findings, deduplicated |
| **Side effects** | Read-only in `report-only` mode; applies `safe_auto` fixes in `autofix` mode |
| **Guard** | Skip for trivial/formatting-only diffs; skip if `ce-init` has not been run first |

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "This diff is small, I can review it quickly" | Small diffs cause big regressions. Every line changed has blast radius. Apply all review dimensions. |
| "I'll skip the skeptical reviewer this time" | The skeptical reviewer catches hallucinations. Skip it = ship false findings. |
| "Security review is overkill for this change" | Auth bypass, secret leaks, and injection bugs start with "harmless" 3-line changes. |
| "I already read this code earlier" | Code drifts. Review against the current diff, not your memory of it. |
| "Performance doesn't matter here" | N+1 queries and O(n²) loops hide in "doesn't matter here" code. Measure. |
| "The tests pass, so it's fine" | Tests passing ≠ no regressions. Tests can be wrong. Review the logic. |
...
1. **Load the `code-reviewing` skill** for its 11-dimension lightweight methodology. Use its dimension prioritization table to focus on what matters for this diff.

2. **Run the harness's built-in code review.** If `$ARGUMENTS` contained a review target (PR number, GitHub URL, or branch name) after stripping recognized tokens, forward that target to the built-in. If no target was provided, run the bare command and let the built-in default to the current branch.
...
3. **Exemption -- no built-in code review exists.** If the current harness has no built-in code review command or skill, do not short-circuit. Continue into the full multi-agent review described in the rest of this skill (Tier 2).

4. **Programmatic callers bypass this short-circuit.** When `mode:autofix`, `mode:report-only`, or `mode:headless` is present, ignore quick intent and run the full multi-agent review. Skill-to-skill callers that want the lightweight pass should invoke `code-reviewing` directly rather than route through this short-circuit.
