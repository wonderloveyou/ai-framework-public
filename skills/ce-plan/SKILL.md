...
| Aspect | Value |
|--------|-------|
| **Input** | Feature description, requirements doc, brainstorm output, or rough idea |
| **Output** | Structured plan doc with tasks, dependencies, acceptance criteria |
| **Side effects** | Creates plan doc in docs/plans/. No code changes. |
| **Guard** | Do not implement. Plan only. Do not plan trivial single-file changes. |

## Planning Mode Selection

This skill handles multiple planning methodologies. Pick the right one:

| Mode | Trigger | Output | Skill |
|------|---------|--------|-------|
| **Universal** (default) | Any task, feature, or idea | `docs/plans/` plan doc | `ce-plan` (this skill) |
| **User Spec** | "user spec", "юзер спек", requirements | `work/{feature}/user-spec.md` | `user-spec-planning` |
| **Tech Spec** | "tech spec", "тех спек", technical spec | `work/{feature}/tech-spec.md` | `tech-spec-planning` |
| **Project Docs** | "project planning", new project docs | `.claude/skills/project-knowledge/` | `project-planning` |
| **Spec-Driven** | Full spec pipeline (user → tech → decompose) | Both specs + tasks | All 3 spec skills in sequence |

If the user asks for a spec or says "спланируй фичу" with spec-first intent → route to the appropriate spec skill.
If the user has a rough idea or non-software task → use Universal mode (stay in this skill).

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "I know what to do, I'll skip the plan" | Plans catch edge cases. 10 minutes planning saves hours of wrong implementation. |
| "This is a small feature, just let me code it" | Small features in complex systems ripple. Map the dependencies before touching code. |
| "I'll plan as I go" | Incremental planning = no planning. The full picture emerges only when you step back. |
| "The user is in a hurry" | Rushed work creates rework. A 5-minute plan is faster than 2 hours of undo. |
...
## Spec-Driven Mode (SDD Pipeline)

When the user explicitly asks for a spec, or when the task is a software feature requiring structured requirements before implementation, route through the SDD pipeline instead of Universal planning:

```
user-spec-planning  →  tech-spec-planning  →  task-decomposition  →  ce-work
```

Each step produces a document that feeds the next. Flags to detect SDD intent:
- "user spec", "tech spec", "project planning", "спланируй фичу"
- Task involves multiple components, external APIs, or new architecture
- Task requires acceptance criteria and formal validation before coding

**Route:**
1. If no user-spec.md exists → run `user-spec-planning`
2. If user-spec exists but no tech-spec → run `tech-spec-planning`
3. If both specs exist → run `task-decomposition` then `ce-work`
4. If starting a new project from scratch → run `project-planning`

Otherwise use Universal planning (stay in this skill).
