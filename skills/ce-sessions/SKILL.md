...
| Aspect | Value |
|--------|-------|
| **Input** | Question/topic string, or empty (prompts user) |
| **Output** | Synthesized findings: what was tried, what didn't work, key decisions, related context |
| **Side effects** | Read-only. Creates temp scratch files, cleans them up |
| **Guard** | Stop if zero relevant sessions found; never read full session files into context |

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "I'll just search git log instead" | Git log shows commits, not investigation dead-ends, failed attempts, or decisions. Sessions capture what git doesn't. |
| "There probably aren't relevant sessions" | Sessions exist for 30+ days. Search before concluding. |
| "The keywords didn't match, so nothing exists" | Branch filter can miss mid-session checkouts. Fall back to keyword search. |
| "I'll read the JSONL files directly" | Session files are 1-7MB. Use extraction scripts. Never read raw. |
| "This old session is probably irrelevant" | Old sessions contain decisions that explain WHY code looks the way it does. |
