...
| Aspect | Value |
|--------|-------|
| **Input** | Issue reference, error message, stack trace, test path, or bug description |
| **Output** | Root cause explanation (full causal chain) + optional fix with test-first discipline |
| **Side effects** | Read-only in diagnosis mode; modifies code only after user confirms in Phase 3 |
| **Guard** | Do not fix anything until causal chain is complete with no gaps |

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "I can see the fix, I'll skip the investigation" | Visible symptoms ≠ root cause. Fixing the symptom leaves the bug alive. Trace the full chain. |
| "One more change and I'll test everything together" | Shotgun debugging. One hypothesis → one change → one test. Always. |
| "The bug is intermittent, probably a race condition" | "Probably" is not a diagnosis. Prove it or keep investigating. |
| "I'll just add a null check and move on" | Null checks hide bugs, they don't fix them. Find why the null got there. |
| "I don't need to reproduce it, I understand from reading the code" | Mental execution is unreliable. Reproduce the bug before fixing. Always. |
| "The environment is probably fine" | Stale deps, wrong branch, missing env vars cause >30% of "mysterious" bugs. Verify environment first. |
