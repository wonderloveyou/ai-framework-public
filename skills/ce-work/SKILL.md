...
| Aspect | Value |
|--------|-------|
| **Input** | Plan document path or bare work description |
| **Output** | Completed implementation with test coverage, linted and verified |
| **Side effects** | Modifies project files, creates commits, may open PR |
| **Guard** | Do not execute without running `ce-init` first; stop on first test failure |

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "I'll add tests after the implementation" | Tests written after are tests written never. TDD catches design problems before they're baked in. |
| "This is a simple change, I don't need a plan" | Simple changes in complex systems are never simple. Map the blast radius first. |
| "I'll fix the lint errors later" | Lint errors compound. Fix them now or ship debt. |
| "One big commit is fine, it's all related" | Atomic commits = atomic reverts. Separate concerns, separate commits. |
| "I know the patterns here, I'll skip reading existing code" | Skip reading = break conventions. Every file you touch, read its neighbors first. |
