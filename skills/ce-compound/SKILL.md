...
| Aspect | Value |
|--------|-------|
| **Input** | Just-solved problem (fresh in session context), optional description |
| **Output** | Structured doc in `docs/solutions/` with YAML frontmatter for searchability |
| **Side effects** | Creates new file in docs/solutions/. No code changes. |
| **Guard** | Do not document trivial fixes (typos, formatting). Only problems that would recur. |

## Anti-Rationalization

| Excuse | Reality |
|--------|---------|
| "I'll document it later" | Later = never. Context fades in hours. Document while it's fresh. |
| "This bug was obvious, no need to document" | Obvious to you today = mysterious to the next person in 3 months. |
| "The solution is in git history" | Git shows WHAT changed. docs/solutions/ shows WHY and HOW to prevent it. |
| "It's just a small fix" | Small fixes carry big lessons. The root cause pattern is what compounds. |
